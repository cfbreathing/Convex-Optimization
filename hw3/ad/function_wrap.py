...
# Used to wrap a function, by currying it.
...
# Avoiding function name changes in wrappers
def wraps(fun, namestr="{fun}", docstr="{doc}", **kwargs):
    def _wraps(f):
        try:
            f.__name__ = namestr.format(fun=get_name(fun), **kwargs)
            f.__doc__ = docstr.format(fun=get_name(
                fun), doc=get_doc(fun), **kwargs)
        finally:
            return f
    return _wraps


# Wrapping a non-array function
...
def get_name(f): return getattr(f, '__name__', '[unknown name]')
def get_doc(f): return getattr(f, '__doc__', '')